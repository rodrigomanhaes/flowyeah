...
- **`review-tree-guard.sh`** — PreToolUse hook on `Bash`. Blocks tree-mutating git commands (`checkout`, `restore`, `switch`, `reset`, `apply`, `am`, `merge`, `rebase`, `pull`, `stash`, `clean`) when invoked from the primary checkout while a `review-state-{N}.md` matches the current branch. Stays out of the way inside build worktrees (`.flowyeah/worktrees/`) and review worktrees (`.flowyeah/review-worktrees/`), in non-flowyeah projects, and on read-only commands like `git fetch`/`git show`/`git blame`. Enforces the "Invariant: Primary Checkout Is Untouched" rule documented in `skills/review/SKILL.md`. Exits 2 with stderr to block, 0 to allow; never errors out (a hook bug must not lock the user out).
