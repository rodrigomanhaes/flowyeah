...
- **`tree-guard.sh`** — PreToolUse hook on `Bash`. Blocks tree-mutating git commands (`checkout`, `checkout-index`, `restore`, `switch`, `reset`, `apply`, `am`, `merge`, `rebase`, `pull`, `stash`, `clean`) when invoked from the primary checkout while a flowyeah session is active. Detection signals, in precedence order: (1) `review-state-{N}.md` matches the current branch; (2) `respond-state-{N}.md` matches the current branch; (3) any `worktrees/<name>/.flowyeah/state.md` exists (build session — no branch correlation, since the primary is by definition on a different branch than any build worktree). Stays out of the way inside build/respond worktrees (`.flowyeah/worktrees/`) and review worktrees (`.flowyeah/review-worktrees/`), in non-flowyeah projects, and on read-only commands like `git fetch`/`git show`/`git blame`. Enforces the "Invariant: Primary Checkout Is Untouched" rule documented in `skills/review/SKILL.md`, `skills/respond/SKILL.md`, and `skills/build/SKILL.md`. The error message names the active session via a session-specific descriptor and points at the right worktree path / exit command. Exits 2 with stderr to block, 0 to allow; never errors out (a hook bug must not lock the user out).
