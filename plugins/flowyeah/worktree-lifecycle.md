...
4. **Write `.envrc`** in the worktree root — one `export KEY=VALUE` line per resolved env var, prefixed with a header comment. Exclude it from version control via the worktree's local git exclude. If `direnv` is on `PATH`, run `direnv allow .` so it activates automatically when the user enters the directory.
...
# Write .envrc for manual shell use
cat > .envrc <<'ENVRC'
...
# Exclude .envrc from version control
WORKTREE_GIT_DIR=$(git rev-parse --git-dir)
mkdir -p "$WORKTREE_GIT_DIR/info"
grep -qF '.envrc' "$WORKTREE_GIT_DIR/info/exclude" 2>/dev/null || echo ".envrc" >> "$WORKTREE_GIT_DIR/info/exclude"
